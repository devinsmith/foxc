...
struct dkHiliteStyle {
  DKColor normalForeColor;            /// Normal text foreground color
  DKColor normalBackColor;            /// Normal text background color
  DKColor selectForeColor;            /// Selected text foreground color
  DKColor selectBackColor;            /// Selected text background color
  DKColor hiliteForeColor;            /// Highlight text foreground color
  DKColor hiliteBackColor;            /// Highlight text background color
  DKColor activeBackColor;            /// Active text background color
  DKuint  style;                      /// Highlight text style
};
...
struct FXTextChange {
  ...
  int   ndel;         /// Number characters deleted at position
  int   nins;         /// Number characters inserted at position
  char *ins;          /// Text inserted at position
  char *del;          /// Text deleted at position
};
...
struct dkText {
  struct dkScrollArea base;
  char *buffer;                    /* Text buffer being edited */
  char *sbuffer;                   /* Text style buffer */
  int         *visrows;            /* Starts of rows in buffer */
  int          length;             /* Length of the actual text in the buffer */
  int          nvisrows;           /* Number of visible rows */
  int          nrows;              /* Total number of rows */
  int          gapstart;            // Start of the insertion point (the gap)
  int          gapend;              // End of the insertion point+1
  int          toppos;              // Start position of first visible row
  int          keeppos;             // Position to keep on top visible row
  int          toprow;              // Row number of first visible row
  int          selstartpos;         // Start of selection
  int          selendpos;           // End of selection
  int          hilitestartpos;      // Hightlight start position
  int          hiliteendpos;        // Hightlight end position
  int          anchorpos;           // Anchor position
  int          cursorpos;           // Cursor position
  int          revertpos;           // Position of cursor prior to dragging
  int          cursorstart;         // Cursor row start pos
  int          cursorend;           // Cursor row end pos
  int          cursorrow;           // Cursor row
  int          cursorcol;           // Cursor column indent (not character offset!)
  int          prefcol;             // Preferred cursor column
  int          margintop;           // Margins top
  int          marginbottom;        // Margin bottom
  int          marginleft;          // Margin left
  int          marginright;         // Margin right
  int          wrapwidth;           // Wrap width in pixels
  int          wrapcolumns;         // Wrap columns
  int          tabwidth;            // Tab width in pixels
  int          tabcolumns;          // Tab columns
  int          barwidth;            // Line number width
  int          barcolumns;          // Line number columns
  struct dkFont *font;                // Text font
  DKColor        textColor;           // Normal text color
  DKColor        selbackColor;        // Select background color
  DKColor        seltextColor;        // Select text color
  DKColor        hilitebackColor;     // Highlight background color
  DKColor        hilitetextColor;     // Highlight text color
  DKColor        activebackColor;     // Background color for active line
  DKColor        numberColor;         // Line number color
  DKColor        cursorColor;         // Cursor color
  DKColor        barColor;            // Bar background color
  int          textWidth;           // Total width of all text
  int          textHeight;          // Total height of all text
  struct dstr       *searchstring;        // String of last search
  DKuint         searchflags;         // Flags of last search
  char  *delimiters;          // Delimiters
  struct dstr       *clipped;             // Clipped text
  int          vrows;               // Default visible rows
  int          vcols;               // Default visible columns
  struct dstr     *help;                // Status line help
  struct dstr       *tip;                 // Tooltip
  struct dkHiliteStyle *hilitestyles;  // Style definitions
  DKuint         matchtime;           // Match time (ms)
  int          grabx;               // Grab point x
  int          graby;               // Grab point y
  DKuchar        mode;                // Mode widget is in
  DKbool         modified;            // User has modified text
};
