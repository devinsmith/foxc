 ...
 *****************************************************************************/
#ifndef FX_SCROLLAREA_H
#define FX_SCROLLAREA_H
...
struct dkScrollArea {
  ...
  /* dkScrollArea virtual functions */
  int (*getViewportHeight) (struct dkWindow *w);
  int (*getViewportWidth) (struct dkWindow *w);
  int (*getContentWidth) (struct dkWindow *w);
  int (*getContentHeight) (struct dkWindow *w);

  struct dkScrollBar    *horizontal;   // Horizontal scroll bar
  struct dkScrollBar    *vertical;     // Vertical scroll bar
  struct dkScrollCorner *corner;       // Scroll corner
  int                    viewport_w;   // Viewport width
  int                    viewport_h;   // Viewport height
  int                    pos_x;        // X scroll position (pos_x<=0)
  int                    pos_y;        // Y scroll position (pos_y<=0)
};
