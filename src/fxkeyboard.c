...
static DKbool wkbAltGrDown(PBYTE ks){
  static DKbool bHasAltGr = FALSE;
  static HKL hklOld = NULL;
  HKL hkl=GetKeyboardLayout(0);
  if(hklOld!=hkl) {
    DKuint ch;
...
DKuint wkbMapKeyCode(UINT iMsg,WPARAM uVirtKey,LPARAM lParam){
  BYTE ks[256];
  char c;
  DKuint i;
  ...
  if(GetKeyboardState(ks)!=0){
    ...
    DKuint xt=HIWORD(lParam)&KF_EXTENDED;
    ...
    switch(uVirtKey){
      ...
      default:
        ...
        for(i=0; i < ARRAYNUMBER(keymapCtl) / 2; ++i) {
        ...
        if('A'<=uVirtKey && uVirtKey<='Z'){
          return (DKuint)(KEYDOWN(ks,VK_SHIFT) ? uVirtKey : uVirtKey-'A'+KEY_a);
          }
...
PFN_TOUNICODEEX dkToUnicodeEx=wkbToUnicodeExStub;
...
int WINAPI wkbToUnicodeExStub(UINT uVirtKey,UINT uScanCode,const BYTE* lpKeyState,LPWSTR pwszBuff,int cchBuff,UINT wFlags,HKL dwhkl){
  ...
  if(osinfo.dwPlatformId==VER_PLATFORM_WIN32_WINDOWS){
    ...
    dkToUnicodeEx = wkbToUnicodeExWin9x;
    }
  else{
    // Windows NT => forward to OS
    HMODULE user32Dll=LoadLibraryA("user32");
    dkToUnicodeEx=(PFN_TOUNICODEEX)GetProcAddress(user32Dll,"ToUnicodeEx");
    if(!dkToUnicodeEx){dkToUnicodeEx=wkbToUnicodeExWin9x;}
    }
  return dkToUnicodeEx(uVirtKey, uScanCode, lpKeyState, pwszBuff, cchBuff, wFlags, dwhkl);
  }
